...
local function hyperTensionEffectStart()
    startRepeatingCallback("blur", random(2000, 5000), function()
    ...
    startRepeatingCallback("ragdoll", random(2000, 5000), function()
    ...
    startRepeatingCallback("vehicle", random(2000, 5000), function()
        Wait(random(1500, 5000))
        DoScreenFadeOut(200)
        if cache.vehicle and cache.seat == -1 then
            SetVehicleSteerBias(cache.vehicle, random(-1.0, 1.0))
        end
        Wait(500)
        DoScreenFadeIn(200)
    end)
